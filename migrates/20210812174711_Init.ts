...
export class Init implements Migrate {
  ...
  async up(
    ...
    builder.createTable("Order").as(builder => [
      builder.column("id", DbType.int64).notNull().identity(0, 1),
      builder.column("orderNo", DbType.string(20)).notNull(),
    ...
    builder.createIndex("IX_Order_OrderNo").on("Order", { "orderNo": 'ASC' });
    builder.setIndexComment("Order", "IX_Order_OrderNo", "OrderNo Index");
