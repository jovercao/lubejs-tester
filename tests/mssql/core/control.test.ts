...
import { executeInProcedure } from '../util';
...
describe('tests/core/control.test.ts', function () {
  ...
  it('if ... elseif ... else ...', async () => {
    ...
    const { rows: rows1 } = await executeInProcedure(db, sql1);
    ...
    const { rows: rows2 } = await executeInProcedure(db, sql2);
    ...
    const { rows: rows3 } = await executeInProcedure(db, sql3);
  ...
  it('set <variant> = ...', async () => {
    ...
    const { rows } = await executeInProcedure(db, sql);
