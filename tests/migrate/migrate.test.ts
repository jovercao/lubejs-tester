...
import { MigrateCli, DbContext, compareSchema, DatabaseSchema, generateSchema } from 'lubejs'
...
describe('Migrate', function () {
  ...
  it('Add & Snapshot', async () => {
    ...
    const initSchema: DatabaseSchema = (await import(initMigrate.snapshotPath)).default
  ...
  it('Update', async () => {
    ...
    const metadataSchema = generateSchema(dbContext)
    // metadataSchema.tables.forEach(table => table.seedData = undefined);
    const difference = compareSchema(cli.targetSchemaName, metadataSchema, dbSchema)
    assert(!difference)
  })
  ...
  it('Sync', async () => {
    ...
    assert(dbSchema)
    const metadataSchema = generateSchema(dbContext)
    const defaultSchema = await cli.getDbSchema()
    // metadataSchema.tables.forEach(table => table.seedData = undefined);
    const difference = compareSchema(cli.targetSchemaName, metadataSchema, dbSchema)
    assert(!difference)
  })
